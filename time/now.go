...
func (now *Now) Monday(strValues ...string) time.Time {
	var parseTime time.Time
	var err error
	if len(strValues) > 0 {
		parseTime, err = now.Parse(strValues...)
		if err != nil {
			panic(err)
		}
	} else {
...
func (now *Now) Sunday(strValues ...string) time.Time {
	var parseTime time.Time
	var err error
	if len(strValues) > 0 {
		parseTime, err = now.Parse(strValues...)
...
func (now *Now) Parse(strValues ...string) (t time.Time, err error) {
	var (
		setCurrentTime  bool
		parseTime       []int
...
func (now *Now) MustParse(strValues ...string) (t time.Time) {
	t, err := now.Parse(strValues...)
	if err != nil {
		panic(err)
	}
	return t
...
// Between check time between the start, end time or not
